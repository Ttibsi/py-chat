...
def client_recv(s: socket.socket, m: Messages) -> None:
    while True:
        try:
...
class Message(Static):
    def on_mount(self) -> None:
        # for msg in m.text: # NOTE: This can't see `m` yet
        #     yield Label(msg)
        while True:
            self.refresh()
...
class Window(App):
    ...
    def on_mount(self) -> None:
        ...
        # NOTE: I think we need to make this global or something to move data
        # between threads
        m = Messages
        threading.Thread(target=client_recv, args=(s, m)).start()
